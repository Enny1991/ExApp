...
inline static void delay_and_sum(double *in1, double *in2, int num, double theta) {
  ...
  if (out_size < num) {
    ...
    if(plan1_forward == NULL)
        __android_log_print(ANDROID_LOG_INFO, LOG_TAG, "Forward1 FALSE");
    plan_backward = fftw_plan_dft_c2r_1d(num, beam_out, beam_out_out, FFTW_ESTIMATE);
    fftw_execute(plan_backward);
    fftw_destroy_plan(plan1_forward);
    fftw_destroy_plan(plan2_forward);
    fftw_destroy_plan(plan_backward);
...
JNIEXPORT jdoubleArray JNICALL Java_com_eneaceolini_fft_FFTW_DelayAndSum(JNIEnv *pEnv, jobject pObj, jdoubleArray in1,  jdoubleArray in2, jdouble theta) {
